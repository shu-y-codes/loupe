# 13 — Compare page (corpus family tiles)
...
stays blocked on 11, not on this slice. Implement Compare after 11 even if 12 has not
started. If 12 writes after this, name the Review / Compare switch. If 12 has already
...
and the only way to compare family cards is to click each contract. The comparison is
already computable from `GET /v1/contracts` + `GET /v1/dq/checks` (no new route). Putting
that table on Review would shove the charts. A sibling page keeps Review intact.
...
|---|---|
| `specs/loupe-ui-design.md` | **Home of the page.** Navigation, Compare sidebar vs Review sidebar, Compare main column (table + optional root chart), click-through, empty store. Add a Compare wireframe. Add the Review / Compare switch to the **existing** Review wireframe; do not otherwise change Review’s main-column order. |
| `specs/loupe-solution-design.md` | Locked decision **6**: keep “No authentication”; replace “one reviewer page, not a view selector” with two pages that are still not persona-shaped. §2 table stays the Review answers; add Compare as the corpus scan. §12: “Two pages,” Review invariants unchanged, Compare pointer to the UI spec. §13 UI tier: two-page assembly, still no persona switch. §15: Compare data flow (loop checks, no `findings[]` grouping). §16 checkbox: Review **plus** Compare. §17: this slice before the README walkthrough. |
...
`sample-corpus.md`. Compare reads the checks envelope slice 11 already owns.
...
1. **Two destinations, sidebar only.** Segmented control under the Loupe title:
   **Review** | **Compare**. Default **Review** so the existing walkthrough still lands
...
3. **Compare is a corpus scan, not a second Review.** Rows = loaded contract × held
...
4. **Shared ingest.** Load demo data, inject, and the ingested-file list stay on both
   pages so Compare is reachable on an empty store without bouncing back.
5. **Click-through.** Selecting a Compare row opens Review with that `contract` and
...
Page copy: **Review** and **Compare**. Do not revive Risk / Trader / Analyst.

## Implementation shape

Streamlit `st.navigation` (or equivalent) with two pages. Shared chrome: title, nav,
demo panel. Review-only chrome stays in `chrome.py` as it is. Compare is a new module
under `src/loupe/ui/` that takes checks envelopes and draws the table — HTTP in the
page, no SQL, no `quality` import.
...
must still pass without visiting Compare.

## Done when

1. Specs in the table above agree: two pages, Review invariants intact, Compare
   behaviour listed, locked decision 6 rewritten, no second truth in §12.
2. Nav: Review | Compare in the sidebar; default Review; Compare does not render
   Review’s cards/charts.
3. Compare table: one row per contract × held grain; four family columns match
...
5. Load demo data / inject / file list work from Compare. Empty store copy points at
   Load demo data.
6. Existing Review `AppTest` suite stays green with the nav present. New tests cover
   Compare assembly, grain-filter if shipped, click-through session keys, and that
   Review still has exactly four family cards and no score line.
7. Browser-pass: land on Review, switch to Compare, open `ESZ25` minute, confirm
...
- Compare path: headers Gaps / Duplicates / Invalid values / Recurring patterns;
  a dual-grain fixture yields two rows; a daily-only fixture yields one.
- Click-through writes `contract` and `quality_grain`.
- Compare does not call `bars_daily` / `vwap` to draw the table.
...
- `src/loupe/ui/` — navigation wrapper, Compare module, shared demo chrome; `chrome.py`
  / `review.py` / `charts.py` only if the nav cannot sit above them without a touch
- `tests/ui/test_pages.py` (Review still), new Compare page tests
...
- Date window on Compare, persona views, apply/override.
...
- `specs/loupe-ui-design.md` (Review law to keep; Compare to add)
